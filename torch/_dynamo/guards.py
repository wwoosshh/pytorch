    ...
    def get_guard_manager_from_source(self, source: Source) -> GuardManager:
        ...
        if istype(source, LocalSource):
            ...
            if not base_guard_manager:  # to make mypy happy
                raise AssertionError("base_guard_manager must not be None")
            idx = source.index
            # NOTE: CPython's PyListRevIter_Type doesn't expose internal sequence indexing in C API
            # without accessing private struct fields, so we use a Python lambda_manager with
            # it.__reduce__(). This incurs an O(N) Python calls cost per guard check across items.
            out = base_guard_manager.lambda_manager(
